...
const CreateArticlePage = () => {
	...
	const handlePdf = async (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		const uploadUrl = `${base}/api/file/save-pdf?folder=articles`;
		if (!token) {
	...
	const onSubmit = async (e: React.FormEvent) => {
		e.preventDefault();
		if (!pdfUrl) {
			toast({ title: 'PDF yuklang', status: 'warning' });
			return;
		}
		const title = title_uz || title_ru || title_en;
		if (!title) {
			toast({ title: "Kamida bitta tilda sarlavha kiriting", status: 'warning' });
			return;
		}
		setSubmitting(true);
		try {
			await AdminArticleService.create({
				collectionId,
				title,
			...
			toast({ title: 'Maqola yaratildi', status: 'success' });
	...
	return (
		<Box py={10}>
			<Box as="form" onSubmit={onSubmit} maxW="md">
				...
				<FormControl mb={4}>
					<FormLabel>Kalit so&apos;zlar — o&apos;zbekcha (vergul bilan)</FormLabel>
					<Input value={keywords_uz} onChange={(e) => setKeywordsUz(e.target.value)} placeholder="ilmiy, maqola, ..." />
				</FormControl>
				<FormControl mb={4}>
					<FormLabel>Kalit so&apos;zlar — ruscha</FormLabel>
					<Input value={keywords_ru} onChange={(e) => setKeywordsRu(e.target.value)} placeholder="научная, статья, ..." />
				</FormControl>
				<FormControl mb={4}>
					<FormLabel>Kalit so&apos;zlar — inglizcha</FormLabel>
					<Input value={keywords_en} onChange={(e) => setKeywordsEn(e.target.value)} placeholder="scientific, article, ..." />
				</FormControl>
				<FormControl mb={4}>
					<FormLabel>PDF fayl</FormLabel>
					<Input type="file" accept=".pdf" onChange={handlePdf} required />
					{pdfUrl && <Box mt={2} fontSize="sm" color="green.600">Yuklandi</Box>}
				</FormControl>
				<FormControl mb={4}>
					<FormLabel>DOI (ixtiyoriy)</FormLabel>
...
export default withAdminLayout(CreateArticlePage);
